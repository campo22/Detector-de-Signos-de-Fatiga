...
public class DriverController {
    ...
    @Operation(summary = "Obtener una lista paginada de todos los conductores con filtros")
    public ResponseEntity<Page<DriverResponse>> getAllDrivers(
            DriverFilterRequest filter,
            @PageableDefault(size = 10, sort = "nombre", direction = Sort.Direction.ASC) Pageable pageable
    ) {

        Page<DriverResponse> driverPage = driverService.getAllDrivers(filter, pageable);
